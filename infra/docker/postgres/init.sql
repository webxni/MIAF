...
-- Set FINCLAW_APP_ROLE_PASSWORD when running prod compose to enable this.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'finclaw_app') THEN
    CREATE ROLE finclaw_app LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE PASSWORD 'change-me-app-role-password';
  END IF;
END
$$;
