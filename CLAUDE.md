...
- Phase 5: web app has protected route scaffolding, login, public `/onboarding`, dashboard/personal/business/documents/settings/audit/agent pages, shared shell/components, and live data wiring for `/personal/budget`, `/personal/debts`, `/personal/goals`, `/personal/investments`, `/business/accounts`, `/business/invoices`, `/business/bills`, `/business/ledger`, `/business/reports`, `/settings`, `/agent`, and `/audit-log`. The Next production build passes in-container (`docker compose exec -T -e NODE_ENV=production web npm run build`); the dev `web` container sets `NODE_ENV=development`, which `next build` must be overridden out of to avoid mixing dev/prod runtimes during prerender. The Dockerfile's `builder` stage pins `NODE_ENV=production`. `/settings` now reads and updates per-user accounting and AI-provider preferences through `GET/PUT /api/settings`; `/agent` is the wired chat surface for `POST /api/agent/chat`, including planned tool calls and confirmation handoff UI; `/audit-log` now reads the owner/admin-gated audit list endpoint with filters, pagination, and inline before/after JSON details. Financial pages remain read-only — most mutations still go through the API directly.
...
- Phase 7: memory backend slice now exists with `app/models/memory.py`, `app/services/memory.py`, `app/schemas/memory.py`, `app/api/memory.py`, and migration `0005_memory.py`; it provides consent-gated durable memory creation, deterministic search, review/expire/forget flows, append-only memory events, deterministic embeddings metadata, a merchant-rule learning path that records corrected CSV draft classifications into `MemoryType.merchant_rule`, and tests.
...
- Wave 5.7: frontend team invite completion — `/settings` now includes owner/admin invite management with pending invite list, one-time link copy, and revoke actions; public `/accept-invite` creates the user session and redirects to `/dashboard`; backend invite creation now returns the one-time token only on create, and invite acceptance enforces the same 12-character password minimum as the rest of auth.
...
* CSV merchant learning now uses the same subsystem: when a user changes an auto-drafted expense account before posting and the posted choice differs from the keyword-only classifier suggestion, `journal.post_entry` records or updates a `merchant_rule` memory (`keywords.merchant` + `keywords.account_code`) so future imports prefer the learned account before keyword matching.
...
* `GET /api/audit-logs` is now a read-only audit surface gated to callers who are owner/admin on at least one entity in their tenant; it always filters by `tenant_id`, supports pagination plus basic field/date filters, and intentionally has no write/update/delete companion routes.
